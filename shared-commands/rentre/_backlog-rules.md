...
모든 에이전트는 백로그 생성 시 이 규칙을 게이트키퍼로 삼아 따릅니다.
...
- **시나리오**: PRD/요구사항을 에픽과 스토리로 분해하여 일감 생성 (예: `/superpowers:writing-plans` 산출물)
...
- **시나리오**: 개발 중 발견한 작업/버그를 즉시 등록 (예: `/superpowers:executing-plans`, `/superpowers:systematic-debugging` 중)
...
### 가설 기반 탐색 — 브레인스토밍 결과
- **주 유형**: Discovery, Epic
- **시나리오**: 브레인스토밍 결과를 탐색 일감으로 변환 (예: `/superpowers:brainstorming` 산출물)
