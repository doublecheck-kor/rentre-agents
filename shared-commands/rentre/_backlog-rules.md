...
### 일감 생성 방식 — **반드시 2단계로 실행**

> ⚠️ **중요**: 템플릿은 *빈 섹션 구조*만 제공합니다. 실제 본문 내용은 에이전트가 직접 채워야 합니다. 1단계만 하고 끝내지 마세요.
...
- 템플릿 적용은 비동기 — 생성 직후 content가 비어 보일 수 있음
...
**Step 2: 본문 채우기 (notion-update-page)**

생성된 페이지에 유형별 필수 섹션을 포함한 실제 본문을 **반드시** 추가합니다.
...
> 🚫 **자주 하는 실수**: Step 1만 실행하고 "등록 완료" 보고하기. 템플릿 섹션은 빈 상자일 뿐이고, 본문 내용은 에이전트가 채워 넣어야 팀원이 작업을 이해할 수 있습니다.

**대안: 템플릿 없이 한 번에 생성**

섹션 구조만 있으면 되고 아이콘 색상은 수동 설정하려면 `template_id` 생략하고 `content` 직접 전달:
```json
{
  "parent": {"data_source_id": "{{NOTION_BACKLOG_DATASOURCE}}"},
  "pages": [{
...
- [ ] **페이지 생성 후 실제 본문이 채워졌는가?** (템플릿 빈 구조만 남기고 끝내지 않았는가?)
- [ ] 유형별 필수 섹션이 모두 실제 내용으로 채워졌는가?
- [ ] "## 목적", "## 작업 내용" 등 섹션 제목만 있고 본문이 비어있지 않은가?
...
- **특이사항**: 본문은 최소 `## 목적` + `## 작업 내용` 두 섹션은 **반드시** 채울 것 (빈 템플릿 금지)
...
| 실수 | 올바른 방법 |
|------|------------|
| 🚫 **템플릿만 적용하고 본문 안 채움** (가장 빈번) | Step 1 생성 → Step 2 `notion-update-page`로 실제 내용 기재. 2단계 절차 필수 |
