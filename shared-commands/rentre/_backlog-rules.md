...
### 일감 생성 방식
**우선: 템플릿 복제** — `notion-duplicate-page`로 템플릿을 복제하면 아이콘, 색상, 섹션 구조가 자동 적용됩니다.
복제 후 `notion-update-page`로 제목, 프로퍼티, content를 채워넣으세요.

**대안: 직접 생성** — 템플릿 복제가 실패하면 `notion-create-pages`로 직접 생성합니다.
