...
Rentre Agents = **BMAD Framework** (개발 엔진) + **Rentre 커맨드** (업무 자동화)
...
| `/rentre:adr` | ADR 분석 | 기술 의사결정 기록 & 검토 (Notion 연동) |

### 연동 & 자동화
| 커맨드 | 용도 | 언제 사용? |
|--------|------|-----------|
| `/rentre:slack` | Slack 연동 | 채널/DM 메시지 전송, 조회 |
| `/rentre:notion` | Notion 검색 | 문서 검색, DB 조회 |
...
```
→ /bmad-party-mode (멀티에이전트 토론)
→ /rentre:challenge (적대적 리뷰로 스트레스 테스트)
→ /rentre:adr (기술 결정 기록)
```
