...
- 카테고리 (select) — AI Automation / AI Tool / BM Experiment / Dev Tool
- 팀 (select) — 개발 / 기획 / 마케팅 / 운영 / CS / 기타
- 작성자 (person) — Notion User ID
- 사용 LLM (multi_select) — Claude / GPT / Gemini / Other
- 태그 (multi_select) — 기존 옵션: Notion, 크롤링, TSX, n8n, PM2, Realtime API
- 데모 링크 (url) — 선택사항
- 등록일 (created_time) — 자동
...
- **카테고리**: 작업 성격에 따라 자동 분류
  - 업무 자동화, 봇 → AI Automation
  - AI 기반 소규모 서비스, 도구 → AI Tool
  - 비즈니스 모델 실험 → BM Experiment
  - 개발 생산성 도구 → Dev Tool
- **팀**: 기본값 "개발" (사용자가 다른 팀이면 변경)
- **사용 LLM**: 기본값 ["Claude"] (다른 LLM 사용했으면 추가)
- **태그**: 기술 스택에서 기존 옵션과 매칭되는 것만. 매칭 안 되면 비워둠
...
프로퍼티 설정:
- 제목, 설명, 카테고리, 팀, 작성자, 사용 LLM, 태그, 데모 링크
